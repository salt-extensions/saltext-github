...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/github/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/github/*")
            ...
            #    "--include=src/saltext/github/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/github/config/schemas",
    )
